        ...
        if($student_payment)
        {
            ...
            try{
                ...
                if($order)
                {
                    ...
                    return view('livewire.backend.razorpay.confirm_paymet',compact('order','jsondata'));
                }

            } catch (\Razorpay\Api\Errors\Error $e) {
                
                return redirect()->route('all_student_payment')->with('alert', ['type' => 'error', 'message' => 'Order Not Created.']);
            }
        }
    }
        ...
        if($student_fine)
        {
            ...
            try{
                ...
                if($order)
                {
                    ...
                    return view('livewire.backend.razorpay.confirm_fine',compact('order','jsondata'));
                }

            } catch (\Razorpay\Api\Errors\Error $e) {
                
                return redirect()->route('all_student_fine')->with('alert', ['type' => 'error', 'message' => 'Order Not Created.']);
            }
        }
    }
    ...
    {  
        ...
        if( $transaction)
        {
            try { 
                ...
                return redirect()->route('all_student_fine')->with('alert', ['type' => 'info','message' => 'This payment has already been fully refunded.',]);
        
            } catch (Exception $e) {

                return redirect()->route('all_student_fine')->with('alert', ['type' => 'error', 'message' => 'An error occurred while processing the refund. Please try again later.']);
            }
        }
    }
